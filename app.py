...
with col1:
    ...
    if data_lengkap:
        ...
        # Garis TOI & AvLOS + Titik Pertemuan
        ...
        # Kotak Legenda In-Graph
        box = FancyBboxPatch((4.2, 10.5), 3.5, 4.0, boxstyle="round,pad=0.2", 
                             facecolor='#ffffff', edgecolor='#cbd5e1', linewidth=1.5, zorder=3)
        ax.add_patch(box)

        ax.text(5.95, 14.1, f"Keterangan — {nama_periode}", fontsize=10, fontweight='bold', color='#0f172a', ha='center', va='top', zorder=4)
    ...
    ax.set_title(f'Grafik Barber Johnson — Periode {nama_periode}', fontweight='bold', fontsize=13, pad=15)
...
with col2:
    ...
    if data_lengkap:
        ...
        st.metric("BOR", f"{bor:.2f}%")
        st.metric("AvLOS", f"{los:.2f} hari")
        st.metric("TOI", f"{toi:.2f} hari")
        st.metric("BTO", f"{bto:.2f} kali")
    else:
        st.info("**Silakan lengkapi semua Form Input Data di Sidebar** untuk menampilkan analisis garis dan status efisiensi.")
